...
def validate(model, dataloader, criterion):
    ...
    with torch.no_grad():
        for (q, q_len, h, h_len, label) in dataloader:
            ...
            for prob in probs:
                all_prob.append([p.item() for p in prob])
            all_labels.extend([num.item() for num in label])
    epoch_time = time.time() - epoch_start
    epoch_loss = running_loss / len(dataloader)
    epoch_accuracy = running_accuracy / (len(dataloader.dataset))
    return epoch_time, epoch_loss, epoch_accuracy, roc_auc_score(all_labels, all_prob, multi_class='ovo')
